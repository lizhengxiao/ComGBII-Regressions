...
IBGReg.solnp <- function(y, X, control = list(print_level = 1,
                                        pars.seed = 113,
                                        pars.init.n = 5, 
                                        tol.rel = 1.0e-8,
                                        print_level = 1,
  ...
  for (j in 1:n.init) {
    ...
    # 判断条件是否符合要求
    while ((random_nums[4] <= log(1))|(random_nums[2] <= log(2))) {
      # 如果不符合要求，重新生成随机数
      random_nums <-  rnorm(4, 0, 1)
    }
  ...
  names(est)[(k+1):(npars)] <- c(paste0('alpha', 1:2), paste0('alpha', 4:5))
...
# model$Hessian
# class <- 'original'
summary.IBG <- function(model, class = c('original', 
                                           'transformed')){
  pars <- model$estimate
  npars <- length(pars)
  k <- npars - 4
  ...
  if(class == 'original'){
    ...
    se <- c(sd[1:k], sd[(k+1):(k+2)], sd[(k+1)], sd[(k+3):(k+4)], 
            sd[(k+2)])
    ...
    var.nu1 <-  t(c(-nu1, nu1))%*%cov[c(1,4), c(1,4)]%*%(c(-nu1, nu1))
    var.nu2 <- NA
...
# 
# summary.IBGReg(model = model, class = 'original')
# summary.IBGReg(model = model, class = 'transformed')
