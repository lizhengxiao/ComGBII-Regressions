...
GBIIGReg.solnp <- function(y, X, control = list(print_level = 1,
  ...
  alpha.init.mat <- matrix(0, nrow = 5, ncol = n.init)
  ...
  for (j in 1:n.init) {
    ...
    tryCatch({
      mReg <- solnp(pars = pars.int,
                    ...
                    ineqLB = c(0, 0),
                    ineqUB = c(10, 10),
                    LB = c(rep(-10, times = dim(X)[2]), rep(-10, times = 5)),
                    UB = c(rep(10, times = dim(X)[2]), rep(10, times = 5)),
  ...
  names(est)[(k+1):(npars)] <- paste0('alpha', 1:5)
...
summary.GBIIG <- function(model, class = c('original', 
  ...
  if(class == 'original'){
    ...
    row.names(summarytable)[1:k] <- paste0('beta', 0:(k-1))
    row.names(summarytable)[(k+1):length(est)] <- c('p1', 'p2', 
                                                    'tau1', 'tau2', 'nu1', 'nu2')
  }
