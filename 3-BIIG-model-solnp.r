...
BIIGReg.solnp <- function(y, X, control = list(print_level = 1,
                                         pars.seed = 113,
                                         pars.init.n = 5, 
                                         tol.rel = 1.0e-8,
                                         print_level = 1,
  ...
  names(est)[(k+1):(npars)] <- paste0('alpha', 2:5)
...
summary.BIIG <- function(model, class = c('original', 
  ...
  if(class == 'original'){
    ...
    se <- c(sd[1:k], NA, sd[(k+1):(k+4)], sd[k+2])
    ...
    var.p1 <- NA
    var.p2 <- p2*cov[1,1]*p2
    var.tau1 <- tau1*cov[2,2]*tau1
    var.tau2 <- t(c(-tau2, tau2))%*%cov[c(1,3), c(1,3)]%*%(c(-tau2, tau2))
    var.nu1 <- nu1*cov[4,4]*nu1
    var.nu2 <- NA
